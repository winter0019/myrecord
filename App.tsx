...
const App: React.FC = () => {
    ...
    if (isAuthenticated) {
      const fetchData = async () => {
        ...
        try {
          ...
          // Fallback to local if needed, but primary is cloud
        } finally {
          setIsLoading(false);
        }
      };
  ...
  const handleAddLoan = async (loan: Loan) => {
    ...
    try {
      ...
      alert("Failed to sync loan with cloud.");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const updateLoanStatus = async (id: string, status: Loan['status']) => {
    ...
    try {
      ...
      alert("Failed to update loan status in cloud.");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const handleBatchAdd = async (newOnes: Contribution[]) => {
    setIsSyncing(true);
    try {
      // For batch uploads, we iterate. In production, use writeBatch
      ...
      alert("Batch import partially failed. Check cloud records.");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const handleDeleteRecord = async (id: string) => {
    if (window.confirm("Delete this ledger entry permanently from cloud?")) {
      setIsSyncing(true);
      try {
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="w-full max-w-md bg-white rounded-[2rem] shadow-2xl p-8 relative z-10 animate-fadeIn">
          <div className="text-center mb-8">
            ...
            <h1 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight">NYSC Katsina State Staff</h1>
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-2">Centralized Society Portal</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block text-center">Cloud Access Key</label>
              <input
                type="password"
                inputMode="numeric"
                maxLength={4}
    ...
    <div className="min-h-screen flex bg-slate-50 text-slate-900 selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <main className={`flex-1 transition-all duration-300 pb-20 md:pb-0 ${isSidebarMinimized ? 'md:ml-20' : 'md:ml-64'}`}>
        <header className="bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-20 flex items-center justify-between px-6 py-3.5">
          ...
          <div className="flex items-center space-x-4">
            {isSyncing && (
              <div className="hidden md:flex items-center space-x-2 bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-100 animate-fadeIn">
                <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></div>
                <span className="text-[10px] font-black text-emerald-600 uppercase tracking-tighter">Saving to Cloud</span>
              </div>
            )}
            <div className="w-9 h-9 bg-emerald-600 rounded-xl flex items-center justify-center font-black text-white shadow-lg text-sm">
              KC
            </div>
          </div>
        </header>
                ...
                <div className="space-y-6 animate-fadeIn">
                  <div className="flex flex-col lg:flex-row justify-between gap-4">
                    <div>
                      ...
                      <p className="text-slate-500 text-xs">Transaction history synced across all devices.</p>
                    </div>
                    <div className="flex items-center space-x-3">
                      <div className="relative flex-1 md:flex-none">
                        ...
                        <input 
                          ...
                          className="pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-2xl outline-none w-full md:w-64 text-sm"
                          value={ledgerSearchTerm}
                          onChange={(e) => setLedgerSearchTerm(e.target.value)}
                        />
                      </div>
                      <button onClick={() => { setEditingRecord(undefined); setIsModalOpen(true); }} className="hidden md:flex bg-emerald-600 text-white px-6 py-3 rounded-2xl font-bold text-sm shadow-xl items-center space-x-2">
                        ...
                        <div className="flex justify-between items-start mb-4">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-slate-400 font-bold">{c.fileNumber}</p>
                            </div>
                          </div>
